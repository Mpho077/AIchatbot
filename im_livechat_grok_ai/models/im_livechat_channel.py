...
class ImLivechatChannel(models.Model):
    ...
    def _ensure_ai_user_online(self, ai_user):
        ...
        # In Odoo 19, presence is managed via mail.presence (One2many on res.users)
        # sudo: mail.presence - need to access presence system for AI user
        presence = self.env['mail.presence'].sudo().search([
            ('user_id', '=', ai_user.id)
        ], limit=1)
    ...
    def _is_livechat_available(self):
        """Override to consider Grok AI as an available operator.

        Without this, the livechat widget won't show on the website when the
        only "operator" is the AI user (which has no real polling activity).
        """
        if self.grok_ai_enabled and self.grok_ai_config_id:
            return True
        return super()._is_livechat_available()
